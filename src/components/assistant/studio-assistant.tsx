...
export function StudioAssistant({settings,packages}:{settings:AssistantSettings;packages:ServicePackage[]}){
  ...
  const logRef=useRef<HTMLDivElement>(null),triggerRef=useRef<HTMLButtonElement>(null);
  const reduce=useReducedMotion();
  useEffect(()=>{const log=logRef.current;if(!open||!log)return;log.scrollTo({top:log.scrollHeight,behavior:"auto"})},[messages,open]);
  useEffect(()=>{if(!open)return;function closeOnEscape(event:KeyboardEvent){if(event.key==="Escape"){setOpen(false);triggerRef.current?.focus()}}window.addEventListener("keydown",closeOnEscape);return()=>window.removeEventListener("keydown",closeOnEscape)},[open]);
