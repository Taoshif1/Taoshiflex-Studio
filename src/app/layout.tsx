...
export default async function RootLayout({children}:{children:React.ReactNode}){const assistant=await getAssistantSettings();const structured={"@context":"https://schema.org","@type":["Organization","ProfessionalService"],name:site.name,url:site.url,description:site.description,areaServed:["Bangladesh","Worldwide"],founder:{"@type":"Person",name:"Gazi Taoshif"},priceRange:"৳৳"};return <html lang="en" data-scroll-behavior="smooth" className={`${sans.variable} ${display.variable}`}><body><a className="skip" href="#main">Skip to content</a><SiteHeader/><main id="main">{children}</main><SiteFooter/><StudioAssistant settings={assistant}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structured).replace(/</g,"\\u003c")}}/></body></html>}
