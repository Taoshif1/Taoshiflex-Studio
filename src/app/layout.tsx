...
export default async function RootLayout({children}:{children:React.ReactNode}){const [assistant,packages]=await Promise.all([getAssistantSettings(),getActivePackages()]);const structured={"@context":"https://schema.org","@type":["Organization","ProfessionalService"],name:site.name,url:site.url,description:site.description,areaServed:["Bangladesh","Worldwide"],founder:{"@type":"Person",name:"Gazi Taoshif"},priceRange:"৳৳"};return <html lang="en" className={`${sans.variable} ${display.variable}`}><body><a className="skip" href="#main">Skip to content</a><SiteHeader/><main id="main">{children}</main><SiteFooter/><StudioAssistant settings={assistant} packages={packages}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structured).replace(/</g,"\\u003c")}}/></body></html>}
