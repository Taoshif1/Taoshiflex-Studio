...
export const metadata = {
  title: "Policies / Client Workspace",
  robots: { index: false, follow: false },
};

export default async function ClientPoliciesPage() {
  const auth = await getClientAuthorization();
  if (!auth) redirect("/client");
  const [policies, maintenance] = await Promise.all([
    getClientPolicies(auth.token),
    ...
    <main className="client-shell">
      <header className="client-head">
        <div>
          ...
          <p>Current policies that apply to your Studio engagement.</p>
        </div>
        <div className="client-head-actions">
          <Link className="action" href="/client/help#policies">Policy guide</Link>
        ...
        {policies.map((policy) => {
          ...
          return (
            <details key={policy.id}>
              <summary><span><strong>{version.title}</strong><small>Version {version.version} / Effective {formatPolicyDate(version.effective_date)}</small></span></summary>
              {version.summary ? <p>{version.summary}</p> : null}
              <PolicyContent content={version.content}/>
            </details>
          );
        })}
        {!policies.length ? <p className="workspace-empty">No Client policies are currently published.</p> : null}
      </div>
    </main>
  );
}
