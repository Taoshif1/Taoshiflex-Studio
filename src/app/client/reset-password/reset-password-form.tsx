...
export function ResetPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      const failure = `Use a password between ${minimumPasswordLength} and ${maximumPasswordLength} characters.`;
      setMessage(failure);
      toast("error", failure);
      return;
    }
    ...
    try {
      ...
      if (response.status === 401) {
        toast("error", "This recovery link is no longer available.");
        setUnavailable(true);
        return;
      }
      if (!response.ok) {
      ...
      toast("success", "Password updated. You can now return to your workspace.");
      setPassword("");
      setConfirmPassword("");
    } catch (error) {
