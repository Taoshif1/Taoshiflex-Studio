...
import { formatMoney, type ProjectBilling } from "@/lib/commercial";
...
export default async function InquiryDetailPage({ params, searchParams }: Props) {
  ...
  const [rows, converted] = await Promise.all([
    supabaseRest<InquiryRecord[]>(`inquiries?id=eq.${encodeURIComponent(id)}&select=*&limit=1`, {}, "privileged").catch(() => []),
    supabaseRest<Array<{ id: string }>>(`client_projects?source_inquiry_id=eq.${encodeURIComponent(id)}&select=id&limit=1`, {}, "privileged").catch(() => []),
  ]);
  const inquiry = rows[0];
  if (!inquiry) notFound();
  const returnPath = safeAdminReturnPath((await searchParams).from, "/studio-admin/inquiries");
  const billing = converted[0]
    ? (await supabaseRest<ProjectBilling[]>(`project_billing?project_id=eq.${converted[0].id}&select=*&limit=1`, {}, "privileged").catch(() => []))[0]
    : undefined;
  const agreedProjectValue = billing
    ? formatMoney(billing.agreed_value_minor, billing.currency, billing.currency_decimals)
    : null;
  ...
  return <main className="admin-shell inquiry-detail-page">
    ...
    <InquiryView inquiry={inquiry} agreedProjectValue={converted[0] ? agreedProjectValue : undefined}/>
