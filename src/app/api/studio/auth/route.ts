...
import { supabaseConfig, verifyStudioAdminToken } from "@/lib/supabase-rest";
...
export async function POST(request:Request){
  ...
  if(!await verifyStudioAdminToken(result.access_token))return Response.json({error:"This account is not a Studio admin."},{status:403});
...
export async function DELETE(request:Request){if(!isSameOrigin(request))return Response.json({error:"Cross-origin request rejected."},{status:403});(await cookies()).delete("studio_access_token");return Response.json({ok:true})}
